...
/**
 * This series of tests loars up arrays to create a cascaring set of retrievals mostly checking
 * and verifying findSubSet retrieval for multiple alias databases.. We will let our sarplesize be dictated by hi and low range values.
	...
	*/
	...
	public static void battery1(Alias alias12) throws Exception {
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine = 0;
			Result arr = (Result) ar.get(j);
			Comparable[] ac = arr.toArray();
			System.out.println("9."+j+") findSubSet("+alias12+",<obj>,<obj>,<obj>) using ="+
					ac[0]+",("+ac[0].getClass()+"),"+
					ac[1]+",("+ac[1].getClass()+"),"+
					ac[2]+",("+ac[2].getClass());
			it = Relatrix.findSubSet(alias12,ac[0],ac[1],ac[2]);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
			...
			displayLine=0;
			//RelatrixHearsetIterator.DEBUG = true;
			System.out.println("10."+j+") findSubSet("+alias12+",*,*,<obj>,String.class, String.class) using range="+ac[2]);		
			it = Relatrix.findSubSet(alias12,'*', '*', ac[2], String.class, String.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			Comparable[] ac = arr.toArray();
			//RelatrixHearsetIterator.DEBUG = true;
			System.out.println("11."+j+") findSubSet("+alias12+",*,<obj>,*, String.class, Long.class) using map="+ac[1]);		
			it = Relatrix.findSubSet(alias12,'*', ac[1], '*',String.class, Long.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
			...
			displayLine =0;
			System.out.println("12."+j+") FindSubset("+alias12+",<obj>,*,*,String.class, Long.class) using domain="+ac[0]);		
			it = Relatrix.findSubSet(alias12,ac[0], '*', '*', String.class, Long.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			displayLine = 0;
			System.out.println("13."+j+") findSubSet("+alias12+",*,<obj>,<obj>,String.class) using map="+ac[1]+" range="+ac[2]);		
			it = Relatrix.findSubSet(alias12,'*', ac[1], ac[2], String.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
			...
			displayLine = 0;
			System.out.println("14."+j+") findSubSet("+alias12+",<obj>,*,<obj>,String.class) using ="+ac[0]+", "+ac[2]);		
			it = Relatrix.findSubSet(alias12,ac[0], '*', ac[2], String.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
			...
			displayLine =0;
			System.out.println("15."+j+") findSubSet("+alias12+",<obj>,<obj>,*, Long.class) using domain="+ac[0]+", map="+ac[1]);		
			it = Relatrix.findSubSet(alias12,ac[0], ac[1], '*',Long.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			Result arr = (Result) ar.get(j);
			Comparable[] ac = arr.toArray();
			System.out.println("16."+j+") findSubSet("+alias12+",*,*,<obj>, String.class, String.class) using range="+ac[2]);		
			it = Relatrix.findSubSet(alias12,'*', '*', ac[2], String.class, String.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
			...
			displayLine =0;
			System.out.println("17."+j+") findSubSet("+alias12+",*,<obj>,*, String.class, Long.class) using map="+ac[1]);		
			it = Relatrix.findSubSet(alias12,'*', ac[1], '*', String.class, Long.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
			...
			displayLine =0;
			System.out.println("18."+j+") findSubSet("+alias12+",<obj>,*,*, String.class, Long.class) using domain="+ac[0]);		
			it = Relatrix.findSubSet(alias12,ac[0], '*', '*', String.class, Long.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			Result arr = (Result) ar.get(j);
			Comparable[] ac = arr.toArray();
			System.out.println("19."+j+") findSubSet("+alias12+",*,<obj>,<obj>, String.class) using map="+ac[1]+" range="+ac[2]);		
			it = Relatrix.findSubSet(alias12,'*', ac[1], ac[2], String.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
			...
			displayLine=0;
			System.out.println("20."+j+") findSubSet("+alias12+",<obj>,*,<obj>,String.class) using domain="+ac[0]+" range="+ ac[2]);		
			it = Relatrix.findSubSet(alias12, ac[0], '*', ac[2], String.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
			...
			displayLine=0;
			System.out.println("21."+j+") findSubSet("+alias12+",<obj>,<obj>,*,Long.class) using domain="+ac[0]+" map="+ac[1]);		
			it = Relatrix.findSubSet(alias12,ac[0], ac[1], '*',Long.class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			Result arr = (Result) ar.get(j);
			Comparable[] ac = arr.toArray();
			System.out.println("22."+j+") findSubSet("+alias12+",*,*,*,<class>,<class>,<obj>,<obj>) using domain="+ac[0].getClass()+" map="+ac[1].getClass()+" range="+lo+" to "+hi);		
			it = Relatrix.findSubSet(alias12,'*','*','*',ac[0].getClass(), ac[1].getClass(),lo,hi);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
			...
			lo+=increment;
			hi+=increment;
			System.out.println("23."+j+") findSubSet("+alias12+",*,*,*,<class>,<class>,<obj>,<obj>) using domain="+((Result)ar.get(j)).getDomain().getClass()+" map="+((Result)ar.get(j)).getMap().getClass()+" range="+lo+" to "+hi);		
			it = Relatrix.findSubSet(alias12,'*','*','*',((Result)ar.get(j)).getDomain().getClass(), ((Result)ar.get(j)).getMap().getClass(),lo,hi);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
			...
			lo+=increment;
			hi+=increment;
			System.out.println("24."+j+") findSubSet("+alias12+",*,*,*,<class>,<class>,<obj>,<obj>) using domain="+((Result)ar.get(j)).getDomain().getClass()+" map="+((Result)ar.get(j)).getMap().getClass()+" range="+lo+" to "+hi);		
			it = Relatrix.findSubSet(alias12,'*','*','*',((Result)ar.get(j)).getDomain().getClass(), ((Result)ar.get(j)).getMap().getClass(),lo,hi);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
