	...
	*/
	...
	public static void battery1(Alias alias12, TransactionId xid2) throws Exception {
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine = 0;
			System.out.println("9."+j+") findTailStream("+alias12+",xid2,<obj>,<obj>,<obj>) using domain="+ar.get(j).getDomain()+",map="+ar.get(j).getMap()+",range="+ar.get(j).getRange());
			RelatrixTransaction.findTailStream(alias12,xid2, ar.get(j).getDomain(), ar.get(j).getMap(), ar.get(j).getRange()).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			System.out.println("10."+j+") findTailStream("+alias12+",xid,*,*,<obj>,String.class, String.class) using range="+ar.get(j).getDomain());		
			RelatrixTransaction.findTailStream(alias12,xid2, '*', '*', ar.get(j).getDomain(), String.class, String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine = 0;
			//RelatrixHeadsetIterator.DEBUG = true;
			System.out.println("11."+j+") findTailStream("+alias12+",xid,*,<obj>,*, String.class, Long.class) using map="+ar.get(j).getDomain());		
			RelatrixTransaction.findTailStream(alias12,xid2, '*', ar.get(j).getDomain(), '*',String.class, Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			displayLine =0;
			System.out.println("12."+j+") findTailStream("+alias12+",xid,<obj>,*,*,String.class, Long.class) using domain="+ar.get(j).getDomain());		
			RelatrixTransaction.findTailStream(alias12,xid2, ar.get(j).getDomain(), '*', '*',String.class, Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			displayLine = 0;
			System.out.println("13."+j+") findTailStream("+alias12+",xid,*,<obj>,<obj>,String.class) using map="+ar.get(j).getDomain()+" range="+ar.get(j).getMap());		
			RelatrixTransaction.findTailStream(alias12,xid2, '*', ar.get(j).getDomain(), ar.get(j).getMap(), String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
			...
			displayLine = 0;
			System.out.println("14."+j+") findTailStream("+alias12+",xid,<obj>,*,<obj>,String.class) using domain="+ar.get(j).getDomain()+", range="+ar.get(j).getMap());		
			RelatrixTransaction.findTailStream(alias12,xid2, ar.get(j).getDomain(), '*', ar.get(j).getMap(), String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine=0;
			System.out.println("15."+j+") findTailStream("+alias12+",xid,<obj>,<obj>,*, Long.class) using domain="+ar.get(j).getDomain()+", map="+ar.get(j).getMap());		
			RelatrixTransaction.findTailStream(alias12,xid2, ar.get(j).getDomain(), ar.get(j).getMap(), '*', Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
					System.out.println(displayLine+"="+c);
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine=0;
			System.out.println("16."+j+") findTailStream("+alias12+",xid2,*,*,<obj>, String.class, String.class) using range="+ar.get(j).getRange());		
			RelatrixTransaction.findTailStream(alias12,xid2, '*', '*', ar.get(j).getDomain(), String.class, String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
			...
			displayLine=0;
			System.out.println("17."+j+") findTailStream("+alias12+",xid,*,<obj>,*, String.class, Long.class) using map="+ar.get(j).getDomain());		
			RelatrixTransaction.findTailStream(alias12,xid2, '*', ar.get(j).getDomain(), '*', String.class, Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
			...
			displayLine=0;
			System.out.println("18."+j+") findTailStream("+alias12+",xid,<obj>,*,*, String.class, Long.class) using domain="+ar.get(j).getDomain());		
			RelatrixTransaction.findTailStream(alias12,xid2, ar.get(j).getDomain(), '*', '*', String.class, Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine=0;
			System.out.println("19."+j+") findTailStream("+alias12+",xid,*,<obj>,<obj>, String.class) using map="+ar.get(j).getDomain()+" range="+ar.get(j).getMap());		
			RelatrixTransaction.findTailStream(alias12,xid2, '*', ar.get(j).getDomain(), ar.get(j).getMap(), String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
			...
			displayLine =0;
			System.out.println("20."+j+") findTailStream("+alias12+",xid,<obj>,*,<obj>,String.class) using domain="+ar.get(j).getDomain()+" range="+ ar.get(j).getMap());		
			RelatrixTransaction.findTailStream(alias12,xid2, ar.get(j).getDomain(), '*', ar.get(j).getMap(), String.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
			...
			displayLine =0;
			System.out.println("21."+j+") findTailStream("+alias12+",xid,<obj>,<obj>,*,Long.class) using domain="+ar.get(j).getDomain()+" map="+ar.get(j).getMap());		
			RelatrixTransaction.findTailStream(alias12,xid2, ar.get(j).getDomain(), ar.get(j).getMap(), '*',Long.class).forEach(o->{
				Result c = (Result)o;
				displayCtrl();
				if(DISPLAY || DISPLAYALL)
