	...
	*/
	...
	public static void battery1AR44() throws Exception {
		...
		if(its != null) {
			...
			while(its.hasNext()) {
				Relation nex = (Relation) its.next();
				pk = (Relation) resolver.getIndexInstanceTable().get(alias1,xid,nex.getIdentity()); 
	...
	public static void battery1AR5() throws Exception {
		...
		if(its != null) {
			while(its.hasNext()) {
				...
				i = resolver.getIndexInstanceTable().get(alias1, xid,(DBKey) nex.getValue()); 
				if( i == null ) {
	...
	public static void battery1AR12() throws Exception {
		...
		if( c != null ) {
			...
			while(its.hasNext()) {
				...
				DBKey db = resolver.getIndexInstanceTable().getKey(alias1,xid,nexe.getKey()); // get the DBKey for this instance integer
				Relation keyset = (Relation) resolver.getIndexInstanceTable().get(alias1,xid,nexe.getValue());
	...
	public static void battery1AR14() throws Exception {
		...
		if(c != null) {
			...
			while(its.hasNext()) {
				Comparable nex = (Comparable) its.next();
				Map.Entry<Relation,DBKey> nexe = (Map.Entry<Relation,DBKey>)nex;
				DBKey db = resolver.getIndexInstanceTable().getKey(alias1,xid,nexe.getKey()); // get the DBKey for this instance 
