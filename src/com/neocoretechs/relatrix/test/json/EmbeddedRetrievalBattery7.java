...
public class EmbeddedRetrievalBattery7 {
	...
	*/
	...
	public static void battery1(String[] argv) throws Exception {
		...
		it =  RelatrixJsonTransaction.findSubSet(xid, '*', '*', '*',xClass, xfClass, RelatrixKVJsonTransaction.getObject(RelatrixKVJson.parseJson(xo50lo),xid), RelatrixKVJsonTransaction.getObject(RelatrixKVJson.parseJson(xo50hi),xid));
		while(it.hasNext()) {
			Object o = it.next();
			Result c = (Result)o;
			displayCtrl();
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			System.out.println("10."+j+") findSubSet(*,*,<obj>,"+xClass+"," +xfClass+") using range="+ar.get(j));		
			it = RelatrixJsonTransaction.findSubSet(xid,'*', '*', ((Result)ar.get(j)).get(3), xClass, xfClass);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine = 0;
			//RelatrixHeadsetIterator.DEBUG = true;
			System.out.println("11."+j+") findSubSet(*,<obj>,*,"+xClass+","+x50Class+") using map="+ar.get(j));		
			...
			displayLine =0;
			System.out.println("12."+j+") FindSubset(<obj>,*,*,"+ xfClass+","+x50Class+") using domain="+ar.get(j));		
		...
		for(int j = 0; j < ar.size(); j++) {
			displayLine =0;
			System.out.println("16."+j+") findSubSet(*,*,<obj>,"+xClass+","+xfClass+") using range="+ar.get(j));		
			...
			displayLine =0;
			System.out.println("17."+j+") findSubSet(*,<obj>,*,"+xClass+","+x50Class+") using map="+ar.get(j));		
			it = RelatrixJsonTransaction.findSubSet(xid,'*', ar.get(j).getDomain(), '*', xClass, x50Class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
			...
			displayLine =0;
			System.out.println("18."+j+") findSubSet(<obj>,*,*,"+xfClass+","+x50Class+") using domain="+ar.get(j));		
			it = RelatrixJsonTransaction.findSubSet(xid,ar.get(j).getDomain(), '*', '*', xfClass, x50Class);
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
		...
		for(int j = 0; j < ar.size(); j++) {
			...
			it = RelatrixJsonTransaction.findSubSet(xid,'*','*','*',ar.get(j).getDomain().getClass(), ar.get(j).getMap().getClass(),RelatrixKVJsonTransaction.getObject(RelatrixKVJson.parseJson(xo50lo),xid),RelatrixKVJsonTransaction.getObject(RelatrixKVJson.parseJson(xo50hi),xid));
			while(it.hasNext()) {
				Object o = it.next();
				Result c = (Result)o;
				displayCtrl();
