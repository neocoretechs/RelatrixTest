...
public class BatteryMorphismJson {
		...
		*/
		...
		public static void battery0(String[] argv) throws Exception {
			...
			for(; i < max; i++) {
				try {
					...
					Relation identity = RelatrixJson.store(jox, xf, jo2);
		...
		public static void battery1AR4(String[] argv) throws Exception {
			int cnt = 0;
			long tims = System.currentTimeMillis();
			Relation prev = (Relation) RelatrixJson.firstKey(Relation.class);
		...
		public static void battery1AR4A(String[] argv) throws Exception {
			int cnt = 0;
			long tims = System.currentTimeMillis();
			DBKey prev = (DBKey) RelatrixKVJson.firstKey(DBKey.class);
			Relation pk = null;
			System.out.println("firstKey="+prev);
			Iterator<?> its = RelatrixKVJson.findTailMapKV((Comparable) prev);
		...
		public static void battery1AR44(String[] argv) throws Exception {
			...
			if(its != null) {
				...
				while(its.hasNext()) {
					Relation nex = (Relation) its.next();
					pk = (Relation) resolver.getIndexInstanceTable().get(nex.getIdentity()); 
		...
		public static void battery1AR5(String[] argv) throws Exception {
			...
			Object i;
			long tims = System.currentTimeMillis();
			Iterator<?> its = RelatrixKVJson.entrySet(Relation.class);
		...
		public static void battery1AR55(String[] argv) throws Exception {
			...
			Object i;
			long tims = System.currentTimeMillis();
			Iterator<?> its = RelatrixJson.entrySet( Relation.class);
		...
		* test size
		...
		public static void battery1AR101(String[] argv) throws Exception {
			int i = max;
			long tims = System.currentTimeMillis();
			long bits = RelatrixKVJson.size(Relation.class);
		...
		public static void battery1AR12(String[] argv) throws Exception {
			int cnt = 0;
			long tims = System.currentTimeMillis();
			Comparable c = (Comparable) RelatrixKVJson.firstKey(Relation.class);
			if( c != null ) {
				...
				while(its.hasNext()) {
					...
					Relation keyset = (Relation) resolver.getIndexInstanceTable().get(nexe.getValue());
		...
		public static void battery1AR14(String[] argv) throws Exception {
			int cnt = 0;
			long tims = System.currentTimeMillis();
			Comparable c = (Comparable) RelatrixKVJson.lastKey(Relation.class);
			if(c != null) {
