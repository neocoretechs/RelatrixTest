 ...
 *
 */
public class BatteryKeysetJsonClientCombo {
	...
	*/
	...
	public static void battery1() throws Exception {
		...
		for(i = min; i < max; i++) {
			...
			rc.store(jox.toString(),UUID.randomUUID());
			rc.store(jo2.toString(),UUID.randomUUID());
			rc.store(xf.toString(),UUID.randomUUID());
			if(DEBUG)
				if(recs % 1000 == 0)
	...
	private static void battery2() throws IllegalAccessException, ClassNotFoundException, IOException {
		...
		jox = new JSONObject(x);
		jo2 = new JSONObject(x50k);
		for(int i = min; i < max; i++) {
			long tim = jox.getLong("timestamp");
			++tim;
			jox.put("timestamp",tim);
			...
			++tim;
			jo2.put("timestamp",tim);
			Iterator<?> it = rc.findTailMapKV(jo2.toString());
			int cnt = 0;
			while(it.hasNext()) {
				Object o = it.next();
				Map.Entry e = (Map.Entry)o;
