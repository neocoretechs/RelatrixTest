 ...
 * and verifying findHeadStream retrieval using the client to a remote {@link com.neocoretechs.relatrix.server.RelatrixTransactionServer}.
 * NOTES:
 * program arguments are  remote_node remote_port_for_database
 * @author Jonathan Groff Copyright (C) NeoCoreTechs 2024
 */
public class StreamRetrievalBatteryTransaction2 {
	...
	private static RelatrixClientTransaction rkvc ;
		...
		public static int div = 10; // divisor to make range within max
		...
		*/
		...
		public static void battery1(String[] argv) throws Exception {
			...
			it =  rkvc.findHeadStream(xid, '*', '*', '*',String.class, String.class, Long.class);
			...
			System.out.println("wildcard results. NOTE: Concrete object references in findHeadStream typically produce null sets.");
			it = null;
			for(int j = 0; j < ar.size(); j++) {
				displayLine =0;
				Comparable[] arel = ((Result)ar.get(j)).toArray();	
				System.out.println("2.) findHeadStream(<obj>,<obj>,<obj>) using ="+
						arel[0]+",("+arel[0].getClass().getName()+"),"+
						arel[1]+",("+arel[1].getClass().getName()+"),"+
						arel[2]+",("+arel[2].getClass().getName());
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findHeadStream(xid, arel[0],arel[1],arel[2]);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				System.out.println("3.) findHeadStream(*,*,<obj>,String.class, String.class) using range="+arel[2]);
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findHeadStream(xid, '*', '*', arel[2], String.class, String.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				displayLine = 0;
				//RelatrixHeadsetStream.DEBUG = true;
				System.out.println("4.) findHeadStream(*,<obj>,*, String.class, Long.class) using map="+arel[1]);
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findHeadStream(xid, '*', arel[1], '*',String.class, Long.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				displayLine =0;
				System.out.println("5.) findHeadStream(<obj>,*,*,String.class, Long.class) using domain="+arel[0]);
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findHeadStream(xid, arel[0], '*', '*', String.class, Long.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				displayLine = 0;
				System.out.println("6.) findHeadStream(*,<obj>,<obj>,String.class) using map="+arel[1]+" range="+arel[2]);
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findHeadStream(xid, '*', arel[1], arel[2], String.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				displayLine = 0;
				System.out.println("7.) findHeadStream(<obj>,*,<obj>,String.class) using domain="+arel[0]+", range="+arel[2]);	
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findHeadStream(xid, arel[0], '*', arel[2], String.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				displayLine =0;
				System.out.println("8.) findHeadStream(<obj>,<obj>,*, Long.class) using domain="+arel[0]+", map="+arel[1]);
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findHeadStream(xid, arel[0], arel[1], '*',Long.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
			...
			System.out.println("9.) findHeadStream(*,*,*,<obj>,String.class,<obj>) using domain to "+fkey2+" map=String.class "+" range to "+hi);		
			it = rkvc.findHeadStream(xid, '*', '*', '*',fkey2,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
				if(DISPLAY)
			...
			System.out.println("10.) findHeadStream(*,*,*,<obj>,String.class,<obj>) using domain to "+fkey2+" map=String.class"+" range to "+hi);		
			it = rkvc.findHeadStream(xid, '*', '*', '*',fkey2,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
			...
			if(it != null)
				rkvc.setStream(it);
			fkey2 = key + String.format(uniqKeyFmt, hi);
			System.out.println("11.) findHeadStream(*,*,*,<obj>,String.class,<obj>) using domain to "+fkey2+" map=String.class"+" range to "+hi);		
			it = rkvc.findHeadStream(xid, '*', '*', '*',fkey2,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
			...
			hi+=(long)(div);
			displayLine =0;
			System.out.println("12.) findHeadStream(*,*,*,String.class,String.class,<obj>) using domain=String.class map=String.class "+" range= to "+hi);
			it = rkvc.findHeadStream(xid, '*', '*', '*',String.class,String.class,hi);
			if(it != null)
				rkvc.setStream(it);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
			...
			if(it != null)
				rkvc.setStream(it);
			System.out.println("13.) findHeadStream(*,*,*,String.class,String.class,<obj>) using domain=String.class map=String.class"+" range to "+hi);		
			it = rkvc.findHeadStream(xid, '*', '*', '*',String.class,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
			...
			if(it != null)
				rkvc.setStream(it);
			displayLine = 0;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   			rkvc.setStream(it);
			System.out.println("14.) findHeadStream(*,*,*,<obj>,<obj>,<obj>) using domain=String.class map=String.class"+" range to "+hi);		
			it = rkvc.findHeadStream(xid, '*', '*', '*',String.class,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
		...
		public static void battery1AR17(String[] argv) throws Exception {
			...
			it.parallel().forEach(e-> {
				...
				System.out.println("KV RANGE 1AR17 KEY SHOULD BE DELETED:"+c.get());
			});
			...
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				System.out.println("Relation:"+c.get());
			});
			siz = rkvc.size(xid);
			if(siz > 0) {
			...
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				System.out.println("DomainRangeMap:"+c.get());
			});
			siz = rkvc.size(xid);
			if(siz > 0) {
			...
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				System.out.println("MapDomainRange:"+c.get());
			});
			...
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				System.out.println("MapRangeDomain:"+c.get());
			});
			...
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				System.out.println("RangeDomainMap:"+c.get());
			});
			...
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				System.out.println("RangeMapDomain:"+c.get());
			});
