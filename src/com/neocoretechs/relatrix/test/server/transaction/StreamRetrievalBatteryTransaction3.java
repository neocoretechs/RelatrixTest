 ...
 * and verifying findTailStream retrieval using the client to a remote {@link com.neocoretechs.relatrix.server.RelatrixTransactionServer}.
 * NOTES:
 * program arguments are  remote_node remote_port_for_database
 * @author Jonathan Groff Copyright (C) NeoCoreTechs 2024
 */
public class StreamRetrievalBatteryTransaction3 {
	...
	private static RelatrixClientTransaction rkvc ;
		...
		*/
		...
		public static void battery1(String[] argv) throws Exception {
			...
			it =  rkvc.findTailStream(xid, '*', '*', '*',String.class, String.class, Long.class);
			...
			for(int j = 0; j < ar.size(); j++) {
				...
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findTailStream(xid, arel[0],arel[1],arel[2]);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findTailStream(xid, '*', '*', arel[2], String.class, String.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findTailStream(xid, '*', arel[1], '*',String.class, Long.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findTailStream(xid, arel[0], '*', '*', String.class, Long.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findTailStream(xid, '*', arel[1], arel[2], String.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findTailStream(xid, arel[0], '*', arel[2], String.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
				...
				if(it != null)
					rkvc.setStream(it);
				it = rkvc.findTailStream(xid, arel[0], arel[1], '*',Long.class);
				it.parallel().forEach(e-> {
					Result c = (Result)e;
					displayCtrl();
					if(DISPLAY)
			...
			System.out.println("9.) findTailStream(*,*,*,<obj>,String.class,<obj>) using domain from "+fkey2+" map=String.class "+" range from "+hi);		
			it = rkvc.findTailStream(xid, '*', '*', '*',fkey2,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
				if(DISPLAY)
			...
			System.out.println("10.) findTailStream(*,*,*,<obj>,String.class,<obj>) using domain from "+fkey2+" map=String.class"+" range from "+hi);		
			it = rkvc.findTailStream(xid, '*', '*', '*',fkey2,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
			...
			if(it != null)
				rkvc.setStream(it);
			fkey2 = key + String.format(uniqKeyFmt, hi);
			System.out.println("11.) findTailStream(*,*,*,<obj>,String.class,<obj>) using domain from "+fkey2+" map=String.class"+" range from "+hi);		
			it = rkvc.findTailStream(xid, '*', '*', '*',fkey2,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
			...
			hi = (long)(max/div);
			displayLine =0;
			System.out.println("12.) findTailStream(*,*,*,String.class,String.class,<obj>) using domain=String.class map=String.class "+" range from "+hi);		
			it = rkvc.findTailStream(xid, '*', '*', '*',String.class,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
				if(DISPLAY)
			...
			if(it != null)
				rkvc.setStream(it);
			System.out.println("13.) findTailStream(*,*,*,String.class,String.class,<obj>) using domain=String.class map=String.class"+" range from "+hi);		
			it = rkvc.findTailStream(xid, '*', '*', '*',String.class,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
			...
			if(it != null)
				rkvc.setStream(it);
			System.out.println("14.) findTailStream(*,*,*,<obj>,<obj>,<obj>) using domain=String.class map=String.class"+" range from "+hi);		
			it = rkvc.findTailStream(xid, '*', '*', '*',String.class,String.class,hi);
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				displayCtrl();
		...
		public static void battery1AR17(String[] argv) throws Exception {
			...
			it.parallel().forEach(e-> {
				Result c = (Result)e;
				Relation dmr = (Relation)((Result)e).get();
				try {
